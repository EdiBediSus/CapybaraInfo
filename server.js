// Slipper Attack Server with Admin Controls (patched)
...
const MAPS = [ /* same map definitions as before */ ];
...
wss.on("connection", ws=>{
    ...
    ws.on("message", raw=>{
        ...
        if(data.type==="setName" && typeof data.name==="string"){
            ...
            // Send state to *all* clients (including the one that requested the change)
            broadcastAll({
                type:"state", 
                playerId: myId, 
                state:{
        ...
        if(data.type==="shoot" && typeof data.angle==="number"){
            ...
            if(now - (me.lastShot || 0) < SHOT_COOLDOWN) return;
            me.lastShot = now;
            const dx = Math.cos(data.angle) * 10;
            const dy = Math.sin(data.angle) * 10;
            bullets.push({ x: data.x || me.x, y: data.y || me.y, dx, dy, t: now });
            return;
        }
    ...
    const interval = setInterval(()=>{
        ...
        if (me.deadUntil > 0 && me.deadUntil <= Date.now()) {
            ...
            // broadcast the respawn to everyone (including the respawned player)
            broadcastAll({
                type:"state", 
                playerId: myId, 
                state:{
        ...
        // Regeneration (server authoritative)
        if(me.deadUntil === 0 && me.hp < 100) { 
            me.hp = Math.min(100, me.hp + 0.5); // regen per tick (50ms tick => ~10 HP/s)
        }

        // Bullet->player collisions & damage (unchanged)
                ...
                bullets.forEach((bullet, bulletIndex) => {
                    ...
                    if(distance < 22) {
                        ...
                        if(target.hp <= 0){
                            ...
                            target.ws.send(JSON.stringify({ type:"dead", playerId: targetId, deadUntil: target.deadUntil }));
                            
                            broadcastAll({ type:"dead", playerId: targetId, deadUntil: target.deadUntil });
        ...
        // *** IMPORTANT CHANGE: send state to everyone INCLUDING the sender so they see server-applied regen/hp ***
        broadcastAll({
            type:"state", 
            playerId: myId, 
            state:{
    ...
    ws.on("close", ()=>{
        ...
        admins.delete(myId);
        players.delete(myId);
        broadcastAll({type:"leave", playerId: myId});
